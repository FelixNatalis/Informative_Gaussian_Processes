...
gp_posterior <- function(data_x, data_y, x_grid,
                         is_combination, kernel_label, kernel_params,
                         sigma_noise = 1e-3, mean_fun = function(x) 0) {
  
  x_grid <- sort(x_grid)
  
  K    <- kernel_wrapper(is_combination, kernel_label, data_x, data_x, params = kernel_params)
  K    <- K + sigma_noise^2 * diag(length(data_x))
  K_s  <- kernel_wrapper(is_combination, kernel_label, data_x, x_grid, params = kernel_params)
  K_ss <- kernel_wrapper(is_combination, kernel_label, x_grid, x_grid, params = kernel_params)
  
  L     <- chol(K)
  alpha <- backsolve(L, forwardsolve(t(L), data_y - mean_fun(data_x)))
  v     <- forwardsolve(t(L), K_s)
  
  mu  <- mean_fun(x_grid) + t(K_s) %*% alpha
  cov <- K_ss - t(v) %*% v
  cov <- make_psd(cov)
  
  list(mu = drop(mu), cov = cov, x_grid = x_grid)
}

gp_posterior_draws <- function(post, n_draws = 100) {
  L <- chol(post$cov)
  m <- matrix(rnorm(length(post$mu) * n_draws), nrow = length(post$mu))
  post$mu + t(L) %*% m
}
